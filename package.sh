...
function package() {
    ...
    printf "[2/2] Package\n"
    if [ ${os} == "windows" ]; then
        zip gvm${release}.${os}-${arch}.zip ./gvm.exe
        shasum -a 256 gvm${release}.${os}-${arch}.zip >> sha256sum.txt
    else
        tar -czv -f gvm${release}.${os}-${arch}.tar.gz ./gvm
        shasum -a 256 gvm${release}.${os}-${arch}.tar.gz >> sha256sum.txt
    fi
}
